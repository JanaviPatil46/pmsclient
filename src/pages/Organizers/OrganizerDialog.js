...
//   // Function to prepare data for submission (used by both auto-save and submit)
//   const prepareSubmitData = (finalSubmit = false) => {
//     const baseSections = organizer?.sections?.map((section) => ({
//       name: section?.text || "",
//       id: section?.id || "",
//       text: section?.text || "",
//       sectionsettings: section?.sectionsettings,
...
//     // Add repeated sections with numeric IDs
//     const repeatedSectionsData = [];
//     Object.keys(repeatedSections).forEach(sectionId => {
//       repeatedSections[sectionId].forEach(repeatId => {
//         const originalSection = organizer?.sections?.find(s => s.id === sectionId);
//         if (originalSection) {
//           repeatedSectionsData.push({
//             name: `${originalSection?.text} (Repeated)` || "",
//             id: repeatId.toString(), // Store as string to match existing pattern
//             text: `${originalSection?.text} (Repeated)` || "",
//             sectionsettings: {
...
//             },
//             formElements:
//               originalSection?.formElements?.map((question) => ({
//                 type: question?.type || "",
//                 id: question?.id || "",
//                 sectionid: repeatId, // This is now a number
//                 options:
//                   question?.options?.map((option) => ({
//                     id: option?.id || "",
//                     text: option?.text || "",
//                     selected: getOptionSelectedState(question, option, repeatId),
//                   })) || [],
//                 text: question?.text || "",
//                 textvalue: getQuestionTextValue(question, repeatId),
//                 questionsectionsettings: question?.questionsectionsettings,
//                 ...(question.type === "File Upload" && {
//                   fileMetadata: {
//                     fileName: uploadedFiles[`${repeatId}_${question.text}`] || "",
//                   },
//                 }),
//               })) || [],
//           });
...
//     return {
//       sections: [...baseSections, ...repeatedSectionsData],
//       status: finalSubmit ? "Completed" : "In Progress",
//       completedby: loginuserid,
//       active: true,
...
//     const allSections = [...visibleBaseSections];

//     // Add repeated sections for visible base sections
